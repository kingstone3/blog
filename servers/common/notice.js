...
export function init(io) {
  ...
  io.use(function(socket, next){
    ...
    if (token) {
      ...
      socket.request.headers.user = {uid: 'user'};

      return next();
    } else {
      return next(new Error('Socket.io token authentication error'));
    }
  });

  Notice = io.on('connection', function(socket) {
    const {user:{uid}={}} = socket.handshake.headers;
